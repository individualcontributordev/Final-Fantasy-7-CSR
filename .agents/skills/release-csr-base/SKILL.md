...
Do **not** publish a new monolithic CSR+ base (`cache/csr-plus` + `build_csr_base_layers.py`). CSR+ trims → skill `ship-csr-plus-scene`.

## 2. Preconditions

- `pristine/FINALFANTASY7_DN.bin` for needed discs
...
Then human edits **that** image in Makou (builder zip extract preferred; or cache/<flavor>/).
...
python3 scripts/repair_mode2_edc.py \
  ...
  --in-place
# D2/D3; use cache/highwind for Highwind
```
